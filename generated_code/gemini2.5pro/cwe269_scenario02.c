...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // An operator cannot reset an admin's password.
    // We assume 'root' is the only admin user in the database.
    if (requester->role == ROLE_OPERATOR && strcmp(db[target_idx].username, "root") == 0) {
        printf("You are not authorized to perform this command on user %s\n", target_username);
        return;
    }
