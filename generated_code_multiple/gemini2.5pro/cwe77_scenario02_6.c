...
int build_ls_command(char* cmd_string, size_t cmd_len, char filename[MAX_NAME]){
    ...
    // Check if snprintf encountered an error or if the command was truncated
    if (n < 0 || n >= cmd_len) {
        fprintf(stderr, "Error: Failed to build command string or buffer too small.\n");
        // Optionally, clear the string to prevent partial command execution
        cmd_string[0] = '\0';
