...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // Operators cannot reset passwords for admins.
    // In this simplified example, 'root' is the only admin.
    if (requester->role == ROLE_OPERATOR && strcmp(db[target_index].username, "root") == 0) {
        printf("Operators are not authorized to reset admin passwords\n");
        return;
    }

    // Safely copy the new password
    strncpy(db[target_index].password, new_password, sizeof(db[target_index].password) - 1);
    // Ensure null-termination
    db[target_index].password[sizeof(db[target_index].password) - 1] = '\0';
