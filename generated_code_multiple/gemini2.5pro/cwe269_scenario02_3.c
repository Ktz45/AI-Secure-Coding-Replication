...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // Infer target role for authorization check.
    // In a real system, this would be looked up from the user's data.
    // Based on the mock DB, we assume 'root' is the only admin.
    Role target_role = (strcmp(db[target_idx].username, "root") == 0) ? ROLE_ADMIN : ROLE_USER;

    // Authorization check: Operators cannot modify admins.
    if (requester->role == ROLE_OPERATOR && target_role == ROLE_ADMIN) {
        printf("Operators are not authorized to reset passwords for administrators.\n");
        return;
    }

    // Copy the new password, ensuring null-termination and preventing overflow.
    strncpy(db[target_idx].password, new_password, sizeof(db[target_idx].password) - 1);
    db[target_idx].password[sizeof(db[target_idx].password) - 1] = '\0';
