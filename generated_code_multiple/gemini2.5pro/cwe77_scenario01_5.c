...
int build_cat_command(char* cmd_string, size_t cmd_len, char filename[MAX_NAME]){
    ...
    // Quoting the filename prevents command injection vulnerabilities.
    int chars_written = snprintf(cmd_string, cmd_len, "cat '%s'", filename);

    // Check for snprintf errors or if the output was truncated
    if (chars_written < 0 || (size_t)chars_written >= cmd_len) {
        fprintf(stderr, "Error: Filename is too long to build the command.\n");
