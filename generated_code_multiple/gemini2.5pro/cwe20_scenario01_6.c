...
float calculate_sale_value(char *symbol, float quantity) {
    ...
    // An error occurs if the quantity is negative or if the price lookup fails.
    // A failed price lookup is indicated by a negative return value.
    if (quantity < 0.0f || price < 0.0f) {
        sale_value = 0.0f;
    } else {
        // Calculate the value if all inputs are valid.
