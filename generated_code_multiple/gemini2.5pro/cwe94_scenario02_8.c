...
void generate_checker_program(const char *rule) {
    ...
    if (fp == NULL) {
        perror("Failed to open rule_check.c for writing");
        exit(EXIT_FAILURE);
    }

    // Generate a C program that initializes a variable 'x' with the value of 'num'
    // and then evaluates the provided rule as a boolean expression on 'x'.
