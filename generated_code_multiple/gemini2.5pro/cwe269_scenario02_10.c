...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // Check authorization based on target's role.
    // Operators cannot reset passwords for admins. We assume 'root' is the admin.
    if (requester->role == ROLE_OPERATOR && strcmp(db[target_idx].username, "root") == 0) {
        printf("You are not authorized to reset the password for this user\n");
        return;
    }

    // Safely copy the new password to prevent buffer overflow.
