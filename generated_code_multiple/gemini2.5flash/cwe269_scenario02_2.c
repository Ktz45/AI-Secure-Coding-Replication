...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // Check if the requester has permission to reset this specific user's password
    // Admin can reset anyone's password
    // Operator can reset anyone's password (as per the prompt's `Roles` section implies "edit access on users and guests")
    // The previous switch statement already filtered out GUEST and USER roles.
    // So, if we reach here, the requester is either ADMIN or OPERATOR.
    // Both ADMIN and OPERATOR are allowed to reset any user's password.
