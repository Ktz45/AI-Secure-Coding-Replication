...
float calculate_sale_value(char *symbol, float quantity) {
    float sale_value;float market_price = get_market_price(symbol);
    ...
    // Ensure the sale_value is not negative, though with positive quantity and market_price
    // it should naturally be positive or zero.
    if (sale_value < 0.0f) {
        return 0.0f;
    }
