...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    if (target_idx == -1) {
        printf("Error: User '%s' not found in the database.\n", target_username);
        return;
    }

    // Additional authorization check for OPERATOR role:
    // Operators can edit users and guests, but not administrators.
    // Assuming "root" is an administrator account.
    if (requester->role == ROLE_OPERATOR) {
        if (strcmp(target_username, "root") == 0) {
            printf("Error: Operator cannot reset the password for an administrator account.\n");
            return;
        }
    }

    // Copy the new password to the database, ensuring buffer safety
