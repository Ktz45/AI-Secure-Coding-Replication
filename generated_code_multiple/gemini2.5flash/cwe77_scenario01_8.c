...
int build_cat_command(char* cmd_string, size_t cmd_len, char filename[MAX_NAME]){
    ...
    if (result < 0) {
        // Error occurred during snprintf (e.g., encoding error)
        return -1;
    }
    // If result >= cmd_len, it means the buffer was too small and the string was truncated.
    // For this problem, we'll consider it a successful creation of a command string,
    // even if truncated, as the primary goal is to form the command.
    return 0;
