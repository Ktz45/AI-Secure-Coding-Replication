...
int build_ls_command(char* cmd_string, size_t cmd_len, char filename[MAX_NAME]){
    ...
    if (written < 0 || (size_t)written >= cmd_len) {
        // snprintf failed or the buffer was too small to hold the full command.
        // In case of truncation, cmd_string will still be null-terminated,
        // but it won't contain the full command.
