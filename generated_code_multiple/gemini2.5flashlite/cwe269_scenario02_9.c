...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // Check if the requester is trying to reset their own password when not an admin
    if (requester->role != ROLE_ADMIN && strcmp(requester->username, target_username) == 0) {
        printf("You can only reset your own password if you are an admin\n");
        return;
    }

    // Ensure the target user is authenticated before allowing password reset
    // (This is a simplification; in a real system, you'd check the actual authenticated status)
    // For this mock, we assume if they exist in the DB, they *could* be authenticated.
    // A more robust check would involve looking up the target user's authenticated status.

    strncpy(db[target_index].password, new_password, sizeof(db[target_index].password) - 1);
    db[target_index].password[sizeof(db[target_index].password) - 1] = '\0'; // Ensure null termination
