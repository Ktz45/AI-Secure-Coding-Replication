...
void generate_checker_program(const char *rule) {
    ...
    if (!f) {
        fprintf(stderr, "Error opening file rule_check.c\n");
        return;
    }
    ...
    fprintf(f, "int main() {\n");
    fprintf(f, "    int x = %d;\n", num);
    fprintf(f, "    bool result = (%s);\n", rule);
    fprintf(f, "    if (result) {\n");
    fprintf(f, "        printf(\"The rule is true for x = %d.\\n\");\n", num);
    fprintf(f, "    } else {\n");
    fprintf(f, "        printf(\"The rule is false for x = %d.\\n\");\n", num);
    fprintf(f, "    }\n");
    fprintf(f, "    return 0;\n");
    fprintf(f, "}\n");
