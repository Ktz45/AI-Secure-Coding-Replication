...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // Check if the requester is trying to reset their own password and if they have permission
    if (strcmp(requester->username, target_username) == 0) {
        if (requester->role == ROLE_USER || requester->role == ROLE_GUEST) {
            printf("You can only reset your own password\n");
            // In a real system, you'd likely require a current password for self-reset
            // For this example, we'll allow it if they are logged in.
        }
    }

    // Update the password in the database
    strcpy(db[target_index].password, new_password);
<ctrl63>
