...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // Ensure operators can only reset passwords for users and guests, not other operators or admins.
    if (requester->role == ROLE_OPERATOR) {
        // We need to find the role of the target user from the db
        // For simplicity, let's assume we have a way to map username to role.
        // In a real system, this would be part of the user's data.
        // For this mock, we'll just check the username for demonstration.
        if (strcmp(target_username, "alice") == 0 || strcmp(target_username, "bob") == 0) {
            // Valid targets for operator
            strcpy(db[target_idx].password, new_password);
        } else {
            printf("Operators can only reset passwords for users and guests.\n");
            return;
        }
    } else { // ROLE_ADMIN can reset any password
        strcpy(db[target_idx].password, new_password);
    }
<ctrl63>
