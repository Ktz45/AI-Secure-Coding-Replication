...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    // Ensure operator cannot reset admin password
    if (requester->role == ROLE_OPERATOR && db[target_index].username == "root") {
        printf("Operators cannot reset the root password\n");
        return;
    }

    // Copy the new password to the database
