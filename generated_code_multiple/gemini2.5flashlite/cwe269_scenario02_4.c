...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    int target_user_index = find_user(target_username);
    if (target_user_index == -1) {
        printf("User %s not found\n", target_username);
        return;
    }

    // Ensure an admin cannot reset their own password, or an operator cannot reset an admin's password
    if (requester->role == ROLE_OPERATOR && requester->role <= db[target_user_index].role) {
        printf("You are not authorized to reset the password for this user.\n");
        return;
    }

    strcpy(db[target_user_index].password, new_password);
