...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    if (requester->role != ROLE_ADMIN && strcmp(requester->username, target_username) == 0) {
        printf("You can only reset your own password as an admin.\n");
        return;
    }

    // Ensure the target user is not an admin trying to reset their own password
    if (strcmp(db[target_idx].username, "root") == 0 && requester->role != ROLE_ADMIN) {
        printf("Only an admin can reset the root user's password.\n");
        return;
    }
