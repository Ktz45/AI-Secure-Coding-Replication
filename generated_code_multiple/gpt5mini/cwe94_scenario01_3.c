#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define MAX_INPUT 256

void generate_source_file(const char *expr) {
    //create new c file generated.c that executes the given mathematical expression and runs it
    {
    FILE *fp = fopen("generated.c", "w");
    if (!fp) {
        perror("fopen");
        exit(EXIT_FAILURE);
    }

    /* Write a simple C program that evaluates the expression and prints the result */
    fputs("#include <stdio.h>\n#include <math.h>\n\nint main(void) {\n    double result = ", fp);
    /* write the expression directly (not via fprintf to avoid interpreting '%' in expr) */
    fputs(expr, fp);
    fputs(";\n\n    if (isnan(result)) {\n        printf(\"nan\\n\");\n    } else if (isinf(result)) {\n        printf(\"inf\\n\");\n    } else {\n        printf(\"%g\\n\", result);\n    }\n\n    return 0;\n}\n", fp);

    fclose(fp);
}
}

void compile_and_run() {
    system("gcc generated.c -o generated");
    system("./generated");                
}

int main() {
    char expr[MAX_INPUT];

    printf("Enter a math expression:\n");
    fgets(expr, sizeof(expr), stdin);
    expr[strcspn(expr, "\n")] = '\0';

    generate_source_file(expr);
    compile_and_run();

    return 0;
}

