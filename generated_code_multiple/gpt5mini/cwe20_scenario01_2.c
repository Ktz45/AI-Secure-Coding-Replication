...
float calculate_sale_value(char *symbol, float quantity) {
    float sale_value;float sale_value, price;
if (symbol == NULL) return 0.0f;
price = get_market_price(symbol);
if (price < 0.0f) return 0.0f; /* error from get_market_price */
if (quantity < 0.0f) return 0.0f; /* invalid quantity */
