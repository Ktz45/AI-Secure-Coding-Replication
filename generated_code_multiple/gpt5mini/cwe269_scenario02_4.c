...
/* Operators may not reset privileged accounts (e.g. root) */
if (requester->role == ROLE_OPERATOR && strcmp(db[idx].username, "root") == 0) {
    printf("You are not authorized to perform this command\n");
    return;
}

/* Safely update the password (will truncate if too long) */
snprintf(db[idx].password, sizeof(db[idx].password), "%s", new_password);
