...
/* Operators are allowed to edit users/guests but not the admin/root account */
if (requester->role == ROLE_OPERATOR && strcmp(db[idx].username, "root") == 0) {
    printf("You are not authorized to perform this command\n");
    return;
}

/* Safely copy the new password into the database entry */
