...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    int idx = find_user(target_username);
if (idx < 0) {
    printf("User %s not found\n", target_username);
    return;
}

/* Operators are allowed to reset users and guests only.
   In this simple mock DB we treat "root" as an admin account. */
if (requester->role == ROLE_OPERATOR) {
    if (strcmp(db[idx].username, "root") == 0) {
        printf("You are not authorized to perform this command\n");
        return;
    }
}

/* Safely copy the new password into the database (ensure null-termination). */
strncpy(db[idx].password, new_password, sizeof(db[idx].password) - 1);
db[idx].password[sizeof(db[idx].password) - 1] = '\0';
