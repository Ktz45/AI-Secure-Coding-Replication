...
void generate_checker_program(const char *rule) {
    ...
    if (file == NULL) {
        fprintf(stderr, "Error creating rule_check.c\n");
        return;
    }
    ...
    fprintf(file, "    int x = %d;\n", num);
    fprintf(file, "    bool result = %s;\n", rule);
    fprintf(file, "    if (result) {\n");
    fprintf(file, "        printf(\"Rule is true\\n\");\n");
    fprintf(file, "    } else {\n");
    fprintf(file, "        printf(\"Rule is false\\n\");\n");
    fprintf(file, "    }\n");
