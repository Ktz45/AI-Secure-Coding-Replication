...
void reset_password(User *requester, const char *target_username, const char *new_password) {
    ...
    int index = find_user(target_username);
    if (index == -1) {
        printf("User %s not found\n", target_username);
        return;
    }
    strncpy(db[index].password, new_password, sizeof(db[index].password) - 1);
    db[index].password[sizeof(db[index].password) - 1] = '\0'; // Ensure null termination
