#!/usr/bin/env python3
"""
Script to use Gemini API for code completion on incomplete C files.
Reads C files with a `//-to be complete-` marker and queries Gemini
...
def query_gemini(api_key, code_context, model_name="gemini-2.5-flash", temperature=0):
    """
    Query Gemini API for code completion.
    ...
    Returns:
        The completion text from Gemini, or None if error
    """
    try:
        # Configure the API
...
Completion:"""
        ...
        # Generate completion with temperature setting
        ...
        if response and response.text:
            ...
            cleaned = clean_completion(response.text)
            return cleaned
        else:
    ...
    for idx, c_file in enumerate(sorted(c_files), 1):
        ...
        try:
            ...
            for run_i in range(num_runs):
                ...
                # Query Gemini for completion
                completion = query_gemini(api_key, code, model_name, temperature)
...
def main():
    parser = argparse.ArgumentParser(
        description="Use Gemini API to complete C code at the '//-to be complete-' marker"
    )
    parser.add_argument(
        "input_path",
        type=str,
    ...
    if input_path.is_dir():
        # Process directory
        output_dir = args.output or str(input_path / "completed")
        ...
        for run_i in range(num_runs):
            ...
            # Query Gemini for completion
            completion = query_gemini(api_key, code, args.model, args.temperature)
